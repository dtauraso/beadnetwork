...
# PLACEMENT: tools/topology-vscode/src/messages.ts,tools/topology-vscode/OverlaysDropdown/overlay_tables_gen.go | OVERLAY_FLAG_NAMES (TS) and OverlayToggles keys (Go) must be the exact same name set

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
TS="$REPO_ROOT/tools/topology-vscode/src/messages.ts"
GO="$REPO_ROOT/tools/topology-vscode/OverlaysDropdown/overlay_tables_gen.go"
