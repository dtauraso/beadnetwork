...
# PLACEMENT: tools/topology-vscode/src/messages.ts,nodes/Wiring/viewstate/panel_state.go | PANEL_FLAG_NAMES (TS) and PanelToggles keys (Go) must be the exact same name set

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
TS="$REPO_ROOT/tools/topology-vscode/src/messages.ts"
GO="$REPO_ROOT/nodes/Wiring/viewstate/panel_state.go"
