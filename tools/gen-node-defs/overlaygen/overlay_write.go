...
func writeOverlayPreamble(w *bufio.Writer, source string, needsFmt bool) {
	...
	if needsFmt {
		...
		fmt.Fprintln(w, "\t\"fmt\"")
		fmt.Fprintln(w)
		fmt.Fprintln(w, "\tT \"github.com/dtauraso/wirefold/tools/topology-vscode/src/Trace\"")
		fmt.Fprintln(w, `)`)
	} else {
		fmt.Fprintln(w, `import (`)
		fmt.Fprintln(w, "\tT \"github.com/dtauraso/wirefold/tools/topology-vscode/src/Trace\"")
...
func writeOverlayStateGen(outPath string, flags []overlayFlag) error {
	...
	writeOverlayPreamble(w, "OVERLAY_FLAG_NAMES in tools/topology-vscode/src/messages.ts.", true)
...
func writeOverlayTablesGen(outPath string, flags []overlayFlag) error {
	var buf bytes.Buffer
	w := bufio.NewWriter(&buf)
	writeOverlayPreamble(w, "OVERLAY_FLAG_NAMES in tools/topology-vscode/src/messages.ts.", false)
