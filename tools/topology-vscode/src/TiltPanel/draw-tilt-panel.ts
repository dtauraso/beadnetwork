...
import {
  COL_STREAM_NODE_ROUNDS_TO_PARALLEL,
  COL_STREAM_NODE_MSGS_TO_PARALLEL,
} from "../Buffer/column-streams-gen";
import { drawPanelBox, panelFont, roundRect } from "../PanelOverlay/panel-box";
import { readF32Run, readI32Run, readU32Run, readText, decodeAt } from "../PanelOverlay/panel-columns";
import {
  COL_STREAM_TILT_PANEL_BOX_X, COL_STREAM_TILT_PANEL_BOX_Y,
  COL_STREAM_TILT_PANEL_BOX_W, COL_STREAM_TILT_PANEL_BOX_H,
  COL_STREAM_TILT_PANEL_START_X, COL_STREAM_TILT_PANEL_START_Y,
  ...
  COL_STREAM_TILT_PANEL_COL_NODE_ROW,
  COL_STREAM_TILT_PANEL_COL_LABEL_TEXT, COL_STREAM_TILT_PANEL_COL_LABEL_LEN,
  COL_STREAM_TILT_PANEL_HEAD_X, COL_STREAM_TILT_PANEL_HEAD_Y,
  COL_STREAM_TILT_PANEL_HEAD_W, COL_STREAM_TILT_PANEL_HEAD_H,
  ...
  COL_STREAM_TILT_PANEL_MSGS_X, COL_STREAM_TILT_PANEL_MSGS_Y,
  COL_STREAM_TILT_PANEL_MSGS_W, COL_STREAM_TILT_PANEL_MSGS_H,
} from "../Buffer/column-streams-gen";
...
function drawCell(
  ...
  c.textBaseline = "middle";
  c.fillStyle = KEY_INK;
  c.font = panelFont(KEY_FONT_PX, 600);
  c.textAlign = "left";
  c.fillText(key, x + CELL_PAD_X, y + h / 2);

  c.fillStyle = VAL_INK;
  c.font = panelFont(VAL_FONT_PX, 700);
  c.textAlign = "right";
...
export function drawTiltPanel(c: CanvasRenderingContext2D): void {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    c.font = panelFont(HEAD_FONT_PX, 700);
    c.textAlign = "center";
    c.textBaseline = "middle";
