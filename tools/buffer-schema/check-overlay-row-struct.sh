...
# PLACEMENT: Buffer/buffer_layout_gen_singletons.go,tools/gen-node-defs/buflayout/buffer_layout_singletons.go | SetOverlayRow must take one named OverlayRow struct, never positional same-typed scalars

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

GEN_FILE="$REPO_ROOT/Buffer/buffer_layout_gen_singletons.go"
