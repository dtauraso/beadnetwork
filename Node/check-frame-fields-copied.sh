...
# PLACEMENT: Node/node_state.go,Node/stream_sink.go | every NodeState field must be named by the adapter that fills it, or it writes a zero into the node file while everything compiles
...
PAIRS = [
    ("Node/nodeactor/nodeframe/node_frame_input.go", "NodeFrameInput",
     "Node/nodeactor/node_geometry_stream.go"),
    ("Node/node_state.go", "NodeState",
     "Node/stream_sink.go"),
]
