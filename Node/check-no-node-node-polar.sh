...
# PLACEMENT: Node/**/*.go,**/*.ts | no DOUBLE-LINK node-node polar record (LocalPolar et al. — the edge's own triple D is the model, not this); a bead streams its world position, never a node-local offset to be summed
...
hits=$(
  find Node NodeKinds Ring/Bead -name '*.go' -print0 2>/dev/null \
    | xargs -0 awk '
        {
          line = $0
          idx = index(line, "//")
