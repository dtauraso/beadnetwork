...
# PLACEMENT: Node/**/*.go,Clock/**/*.go | no time.Sleep/time.After/time.NewTicker outside Clock/clock.go; block on the tick channel instead
set -euo pipefail
...
while IFS= read -r line; do
  case "$line" in
    WAIT)
      ...
      echo "found outside Clock/clock.go. A goroutine parked here cannot service its other"
