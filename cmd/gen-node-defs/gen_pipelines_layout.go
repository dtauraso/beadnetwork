...
func generateCurveParams(repoRoot string) {
	...
	curveParamsTsPath := filepath.Join(srcRoot(repoRoot), "Buffer", "curve-params.ts")
...
func generateOverlayGen(repoRoot string) {
	messagesTSPath := filepath.Join(srcRoot(repoRoot), "messages.ts")
	overlayFlags, err := overlaygen.ParseOverlayFlags(messagesTSPath)
	if err != nil {
		fatalf("parse overlay flags: %v", err)
	}
	viewstateDir := filepath.Join(srcRoot(repoRoot), "OverlaysDropdown")
...
func generateShadingParams(repoRoot string) {
	...
	shadingParamsTsPath := filepath.Join(srcRoot(repoRoot), "Buffer", "shading-params.ts")
...
func generateBufferLayout(repoRoot string) {
	...
	bufLayoutGenGoPath := filepath.Join(srcRoot(repoRoot), "Buffer", "buffer_layout_gen.go")
	bufLayoutGenGoRowsPath := filepath.Join(srcRoot(repoRoot), "Buffer", "buffer_layout_gen_rows.go")
	bufLayoutGenGoRows2Path := filepath.Join(srcRoot(repoRoot), "Buffer", "buffer_layout_gen_rows2.go")
	bufLayoutGenGoSingletonsPath := filepath.Join(srcRoot(repoRoot), "Buffer", "buffer_layout_gen_singletons.go")
	...
	schemaDir := filepath.Join(srcRoot(repoRoot), "Buffer")
...
func generateFrameTags(repoRoot string) {
	frameTagsGoPath := filepath.Join(srcRoot(repoRoot), "Buffer", "frame_tags.go")
	frameTagsHeader, frameTagConsts, err := parseFrameTags(frameTagsGoPath)
	if err != nil {
		fatalf("parse frame tags: %v", err)
	}
	frameTagsTSPath := filepath.Join(srcRoot(repoRoot), "Buffer", "frame-tags.ts")
...
func generateInputLayout(repoRoot string) {
	...
	inputLayoutTSPath := filepath.Join(srcRoot(repoRoot), "schema", "input", "input-layout-gen.ts")
