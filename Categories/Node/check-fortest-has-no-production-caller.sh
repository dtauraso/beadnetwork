...
# PLACEMENT: Categories/Node/**/*.go | a *ForTest symbol must have no production (non-_test.go) caller
...
if files_scanned == 0:
    raise SystemExit(
        ...
        "  a clean scan of zero files for a whole day after nodes/ became Categories/Node/; a bare\n"
        "  directory name is invisible to check-guard-paths-exist.sh. Repoint root."
    )

out = []
out.append(f"SCANNED {files_scanned} Categories/Node/**/*.go production files, {len(symbols)} *ForTest symbol(s): {', '.join(sorted(symbols)) if symbols else '(none found)'}")
if hits:
    out.append("HITS")
    out += hits
print("\n".join(out))
