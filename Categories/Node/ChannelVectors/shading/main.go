...
func main() {
	genpaths.SetName("Categories/Node/ChannelVectors/shading")
	...
	if err := params.WriteShadingParams(tsPath, shParams, genpaths.Name(), "Categories/Node/ChannelVectors/shading_params.go"); err != nil {
		genpaths.Fatalf("write %s: %v", tsPath, err)
	}
	genpaths.Announce(tsPath, len(shParams), "channel constants")
}
