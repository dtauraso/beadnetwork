...
func main() {
	genpaths.SetName("Categories/Node/Edge/shading")
	...
	if err := params.WriteShadingParams(tsPath, shParams, genpaths.Name(), "Categories/Node/Edge/shading_params.go"); err != nil {
		genpaths.Fatalf("write %s: %v", tsPath, err)
	}
	genpaths.Announce(tsPath, len(shParams), "edge constants")
}
