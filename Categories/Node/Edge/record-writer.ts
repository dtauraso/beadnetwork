import { IN_KIND_EDIT_UPDATE, IN_UPDATE_ATTRS, IN_UPDATE_KINDS } from "./wire-gen";
...
export function attrIndex(attr: string): number {
  ...
  if (i < 0) {
    throw new Error(
      `attrIndex: no wire byte exists for update attribute "${attr}" — Node/Edge/update_attrs.go does ` +
