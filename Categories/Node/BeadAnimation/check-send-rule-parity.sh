...
# PLACEMENT: **/*.go,Categories/Node/*.go,Categories/Node/BeadAnimation/send-rule.ts | a new SendRule const must also appear in the SEND_RULES array in send-rule.ts, which sits beside send_rule.go

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

WIRING_DIR="$REPO_ROOT"
WIRE_DIR="$REPO_ROOT/Categories/Node"
TYPES_TS="$REPO_ROOT/Categories/Node/BeadAnimation/send-rule.ts"
...
assert_nonempty "$GO_RULES" "SendRule consts (Categories/Node/BeadAnimation/send_rule.go)"
