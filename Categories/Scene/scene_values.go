...
func SceneValuePath(sceneRoot, name string) string {
	...
	panic("Scene.SceneValuePath: " + name + " is not a scene value, so Go and the renderer disagree about where it lives; add it to SceneValues in Categories/Scene/scene_values.go")
}
