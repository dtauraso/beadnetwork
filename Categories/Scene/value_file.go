...
func ReadIfExists(path string, dst any) bool {
	...
	if err := decodeLeaf(raw, dst); err != nil {
		fmt.Fprintf(os.Stderr, "valuefile: CORRUPT LEAF: %s: %v\n", path, err)
		return false
	}
	return true
}

func decodeLeaf(raw []byte, dst any) error {
	switch p := dst.(type) {
	case *bool:
		if len(raw) != 1 {
...
func readLeafInt64(raw []byte) (int64, error) {
	if len(raw) != 8 {
