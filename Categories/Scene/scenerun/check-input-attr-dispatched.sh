...
# PLACEMENT: Categories/Input/Stdin/edit_update_decode.go,*/edit_decode.go,Categories/Scene/scenerun/dispatch_edit.go | a new addressed-edit attribute must reach a handler, not just decode off the wire
...
ALL_GO_FILES=$(git ls-files '*.go' 2>/dev/null | while IFS= read -r p; do [ -f "$p" ] && printf '%s ' "$p"; done)
