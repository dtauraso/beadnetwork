...
func main() {
	genpaths.SetName("Categories/Scene/scenebuild/kindports")
	repoRoot, srcRoot := genpaths.Roots()
	kinds := Kinds(repoRoot)

	outPath := filepath.Join(srcRoot, "Scene", "scenebuild", "kind_ports_gen.go")
...
func writeKindPorts(outPath string, kinds []KindEntry) error {
	...
	fmt.Fprintln(w, `// GENERATED by Categories/Scene/scenebuild/kindports — do not edit.`)
	fmt.Fprintln(w, `// Source: Categories/NodeKinds/<Kind>/SPEC.md ## Ports table.`)
	fmt.Fprintln(w, `// Regenerate with: go generate ./...`)
	fmt.Fprintln(w)
	fmt.Fprintln(w, `package scenebuild`)
	fmt.Fprintln(w)
