...
func main() {
	genpaths.SetName("Categories/Scene/Tori/shading")
	repoRoot, srcRoot := genpaths.Roots()

	dir := filepath.Join(srcRoot, "Scene", "Tori")
	...
	if err := params.WriteShadingParams(tsPath, shadingParams, genpaths.Name(), "Categories/Scene/Tori/shading_params.go"); err != nil {
		genpaths.Fatalf("write %s: %v", tsPath, err)
	}
	genpaths.Announce(tsPath, len(shadingParams), "torus constants")
}
