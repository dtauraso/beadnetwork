...
- **Coplanar normal**: a quarter turn (`Wiring.QuarterTurnPhiIdx`, 6 steps of
  ...
  sign. `coplanarNormal` (`Categories/NodeKinds/PairNode/vectors.go`) reads it straight off the tilt's own
  `quarter` link on the ring (`Categories/NodeKinds/PairNode/ring.go`) — the ring is built with that link
