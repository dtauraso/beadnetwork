...
# PLACEMENT: Categories/NodeKinds/*/node.go,Categories/NodeKinds/*/*.go | a node-kind package may import only bead/nodeapi/clock/kindreg/portwiring, never a sibling kind — the gate, the held driver and the build args all live in the kinds that use them
...
is_spine() { [ "$1" = "bead" ] || [ "$1" = "spatial" ] || [ "$1" = "rowevent" ] || [ "$1" = "nodeapi" ] || [ "$1" = "clock" ] || [ "$1" = "kindapi" ] || [ "$1" = "kindreg" ] || [ "$1" = "portwiring" ]; }
