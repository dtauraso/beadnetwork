...
# PLACEMENT: Categories/NodeKinds/*/node.go,Categories/NodeKinds/*/*.go | a node-kind package may import only bead/clock, never a sibling kind and no shared kind package — the gate, the held driver, the port bindings and the build args all live in the kinds that use them
