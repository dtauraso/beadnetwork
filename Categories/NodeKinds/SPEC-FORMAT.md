...
Each `Categories/NodeKinds/<Kind>/SPEC.md` is the canonical description of one Go node kind. It is the source of truth that drives the Go runtime (firing rule + struct), the TSX render (port positions + label), and the AI-assisted parity check.
...
- `shape`, `fill`, `stroke`, `width`, `height` — optional `NodeTypeDef`-compatible
  ...
  `Categories/Node/nodegeom/node_dims_gen.go` (used for port-to-port arc length), falling back to
  110×60 if omitted.
