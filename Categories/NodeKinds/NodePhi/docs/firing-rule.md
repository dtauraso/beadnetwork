...
(`Categories/NodeKinds/NodePhi/self.go`)) — there is **no separate `NodeMover` goroutine** for
either node of a pair (`Categories/Scene/Dispatch/movers.go` never constructs one for an id
claimed via `ClaimSelfDrive`). `SyncTiltIndex`/`SyncReceivedVector`/`ClearOutBeads` below
are therefore plain method calls on this same object (`NodePhiSelf.SetTiltIndex`/
...
mover) carries THREE distinct edits, applied by `applyTiltEdit` (`Categories/NodeKinds/NodePhi/edits.go`):
...
- **START** (`TiltEditMsg.Start`, the START TILT button, `TiltVectorButtons.tsx`): opens
  ...
  Pairing two NodePhi instances with one edge running each direction (a.Out → b.In,
