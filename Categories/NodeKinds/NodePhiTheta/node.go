...
var (
	sendA = make(chan TiltPanel.TiltVectorMsg)
	sendB = make(chan TiltPanel.TiltVectorMsg)
	built int
)
...
func (n *NodePhiTheta) breadcrumb(label, value string) {
		...
		if clk.Speed() > 0 {
			...
			for !sent || !got {
				...
				case sent:
					select {
					case v := <-n.VectorIn:
						arrival, got = Turn{Phi: int(v.PhiIdx), Theta: int(v.ThetaIdx), R: int(v.RIdx)}, true
					case <-ctx.Done():
						return
					}
				case got:
					select {
					case n.VectorOut <- mine:
						sent = true
					case <-ctx.Done():
						return
					}
...
var Builder = BuilderFor("NodePhiTheta",
	func(a BuildArgs) (any, error) {
		...
		if built == 0 {
			n.VectorOut, n.VectorIn = sendA, sendB
		} else {
			n.VectorOut, n.VectorIn = sendB, sendA
		}
		built++
