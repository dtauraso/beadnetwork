...
# PLACEMENT: Categories/NodeKinds/*/node.go | a kind's wire.Register("<Kind>") name must be unique across every node package
set -euo pipefail
...
shopt -s nullglob
node_files=("$REPO_ROOT"/Categories/NodeKinds/*/node.go)
shopt -u nullglob
