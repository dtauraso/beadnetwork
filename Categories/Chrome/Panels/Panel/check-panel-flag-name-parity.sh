...
# PLACEMENT: Categories/Chrome/Panels/Panel/flags.ts,Categories/Chrome/Panels/Panel/panel_state.go | PANEL_FLAG_NAMES (TS) and PanelToggles keys (Go) must be the exact same name set

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
TS="$REPO_ROOT/Categories/Chrome/Panels/Panel/flags.ts"
GO="$REPO_ROOT/Categories/Chrome/Panels/Panel/panel_state.go"
...
if [ "$ts_names" != "$go_names" ]; then
  ...
  diff <(printf '%s\n' "$ts_names") <(printf '%s\n' "$go_names") || true
  echo "If you changed the panel vocabulary, edit PANEL_FLAG_NAMES in Categories/Chrome/Panels/Panel/flags.ts and"
