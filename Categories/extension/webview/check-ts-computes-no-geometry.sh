...
# PLACEMENT: Categories/extension/webview/**/*.ts,Categories/extension/webview/**/*.tsx | TS plots what Go streams: no bead positions, no edge curves, no traversal timing computed here
set -euo pipefail
...
SRC_DIR="$REPO_ROOT/Categories/extension/webview"
...
for token in "${FORBIDDEN[@]}"; do
  while IFS= read -r line; do
    ...
    HITS=$((HITS + 1))
  done < <(grep -arn --include="*.ts" --include="*.tsx" "$token" "${TS_ROOTS[@]}" 2>/dev/null || true)
done
