...
# PLACEMENT: Categories/extension/webview/**/*.ts,Categories/extension/webview/**/*.tsx | only buffer-decode-edge.ts/buffer-layout.ts may read EdgeLabelOff/Len; the renderer must not

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
WEBVIEW_DIR="$REPO_ROOT/Categories/extension/webview"
