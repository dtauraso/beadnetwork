...
# PLACEMENT: Categories/extension/webview/**,Categories/Node/**,Categories/Scene/**,Categories/Ring/**,Categories/Chrome/** | Go-owned shading props (roughness, ior, etc.) must reference shading-params.ts, not a numeric literal

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

SCAN_DIR=(
  "$REPO_ROOT/Categories/extension/webview"
