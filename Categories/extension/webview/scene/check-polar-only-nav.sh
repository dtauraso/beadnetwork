...
# PLACEMENT: Categories/extension/webview/scene/interaction-controls.ts | the nav handler does no Cartesian math; angle/sphere math lives in the polar helpers

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
NAV_DIR="$REPO_ROOT/Categories/extension/webview/scene"
