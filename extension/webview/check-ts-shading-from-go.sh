...
# PLACEMENT: extension/webview/**,Node/**,Scene/**,Ring/**,Chrome/** | Go-owned shading props (roughness, ior, etc.) must reference shading-params.ts, not a numeric literal

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

SCAN_DIR=(
  "$REPO_ROOT/extension/webview"
  "$REPO_ROOT/Node"
  "$REPO_ROOT/Scene"
  "$REPO_ROOT/Ring"
  "$REPO_ROOT/Chrome"
...
if ! grep -arq --include='*.ts' --include='*.tsx' '/nodegeom/shading-params"' "${SCAN_DIR[@]}"; then
  echo 'ts-shading-from-go: three/ does not import from `Node/nodegeom/shading-params.ts` — shading params must come from Go'
  HITS=$((HITS + 1))
fi
...
echo ""
echo "ts-shading-from-go: $HITS hit(s) — shading parameter VALUES must live in Go (Node/nodegeom/shading_params.go), not TS"
exit 1
