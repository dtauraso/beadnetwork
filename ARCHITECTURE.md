...
─────────────────────                ─────────────────
  ...
  extension/runCommand.ts                  extension/webview/scene/ThreeView.tsx
  extension/handle-message.ts    extension/webview/scene/scene-root.tsx
  extension/html.ts                          Node/node-label.ts
  extension/goBuild.ts                     Scene/scene-leaves.ts
  src/* (shared)
```
...
`extension/messages.ts` is the shared discriminated-union source for both sides.
`WebviewToHostMsg` includes `ready` and the binary bridge envelope (a fully
encoded editor→Go record built by the concern that owns the edit — `Overlay/encode.ts`,
`Scene/encode.ts`, `Node/encode.ts` and their siblings, each beside that concern's
...
`extension/handle-message.ts`. Per CLAUDE.md, Go → TS is the block
...
attr, and overlay flag, defined in `Input/gen/input_layout_declared.go`. The TS side
(`Node/wire-gen.ts`) is GENERATED from that Go string by
...
and so cannot be trusted. (Removed kind bytes are preserved as GAPS in `Input/gen/input_layout_declared.go` and
never renumbered.)
...
| `extension/handle-message.ts` | Routes `WebviewToHostMsg` to the Go process / disk |
| `extension/html.ts` | Webview HTML shell + CSP |
...
| registries | There is no `schema/` and no `Buffer/`: each registry lives with its concern — `node-defs.ts` in `NodeKinds/`, `wire-defs.ts` in `Scene/loadspec/`, the input codec in `Input/`, the trace events with each owner |
...
| `Scene/scene-leaves.ts` | Polls the scene's binary leaves, including `spawn`, whose change means Go was replaced |
| `extension/webview/scene/scene-root.tsx` | Composition root of the render tree; each component it assembles reads its own block files |
| `extension/webview/scene/ThreeView.tsx` | R3F `<Canvas>` root. Holds NO gesture state — raw pointer/wheel events forward verbatim to Go's FSM (`Input/Gesture` package) |
| `Input/Drag/raw-input-build.ts` | Raw pointer/wheel + raycast hit → binary `raw-input` record to Go |
| `Overlay/overlay-flags.ts` | Read-only reflection of Go-owned overlay-toggle state (`useSyncExternalStore`; no store) |
...
- **The `topology/` tree** — read directly by the Go loader (`Scene/scenebuild/load.go`,
...
- **`<tree-root>/view/{camera,overlays,sphere}.json`** — one file per writer, for
  ...
  `Scene/scenepaths/scene_paths.go`. (An earlier shared sidecar under that same `view/`
