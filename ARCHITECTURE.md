...
─────────────────────                ─────────────────
  ...
  src/Host/runCommand.ts                  src/webview/three/scene/ThreeView.tsx
  src/Host/handle-message.ts    src/webview/three/scene/buffer-scene.tsx
  src/Host/html.ts              src/webview/three/decode/buffer-decode-{view,edge,node,interior}.ts
...
`src/schema/messages.ts` is the shared discriminated-union source for both sides.
