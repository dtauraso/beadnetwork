...
# PLACEMENT: Overlay/flags.ts,Overlay/overlay_tables_gen.go | OVERLAY_FLAG_NAMES (TS) and OverlayToggles keys (Go) must be the exact same name set

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
TS="$REPO_ROOT/Overlay/flags.ts"
GO="$REPO_ROOT/Overlay/overlay_tables_gen.go"
...
if [ "$ts_names" != "$go_names" ]; then
  ...
  diff <(printf '%s\n' "$ts_names") <(printf '%s\n' "$go_names") || true
  echo "If you changed the overlay vocabulary, edit OVERLAY_FLAG_NAMES in Overlay/flags.ts and"
