...
func BuildEdgeStreamFrame(tick uint32, sx, sy, sz, ex, ey, ez float32, label string, events []StreamEvent) []byte {
	...
	B.SetEdgeRow(buf[off:off+B.BufEdgeStride], 0, sx, sy, sz, ex, ey, ez, 0, uint32(len(labelBytes)))
	off += B.BufEdgeStride
