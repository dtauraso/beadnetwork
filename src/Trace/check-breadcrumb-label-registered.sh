...
LABEL_FN_FILE=$(grep -rl '^func breadcrumbLabelFor(' --include="*.go" src/Node/bead 2>/dev/null | head -1 || true)
if [[ -z "$LABEL_FN_FILE" ]]; then
  echo "check-breadcrumb-label-registered: MISCONFIGURED — breadcrumbLabelFor not found in" >&2
  echo "src/Node/bead; it was renamed or deleted. The switch this guard exists to keep in sync" >&2
