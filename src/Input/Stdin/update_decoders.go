package Stdin

import (
	"fmt"

	"github.com/dtauraso/wirefold/src/Input/Codec"
)

type UpdateDecoder func(r *Codec.Reader, attr byte) (StdinMsg, bool)

var updateDecoders = map[string]UpdateDecoder{}

func RegisterUpdateDecoder(entity string, fn UpdateDecoder) {
	if fn == nil {
		panic("Stdin.RegisterUpdateDecoder: nil decoder for " + entity)
	}
	if _, dup := updateDecoders[entity]; dup {
		panic("Stdin.RegisterUpdateDecoder: " + entity + " registered twice; two packages claim the same entity")
	}
	updateDecoders[entity] = fn
}

func AssertUpdateDecodersComplete() {
	var missing []string
	for _, entity := range Codec.InUpdateKinds {
		if _, ok := updateDecoders[entity]; !ok {
			missing = append(missing, entity)
		}
	}
	if len(missing) > 0 {
		panic(fmt.Sprintf(
			"Stdin.AssertUpdateDecodersComplete: no decoder registered for %v. The wire can carry "+
				"an update for each of these (Codec.InUpdateKinds, from INPUT_LAYOUT_FINGERPRINT), so every "+
				"edit naming one would decode to nothing and the affordance would look dead. The owning "+
				"package registers in init(); if that package is imported by nothing, its init() never runs.",
			missing))
	}
}
