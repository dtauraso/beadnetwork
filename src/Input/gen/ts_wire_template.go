  ...
  if (i < 0) {
    throw new Error(
      ` + "`" + `enumIndex: "${s}" is not in the wire enum [${list.join(",")}] — every caller passes a name ` + "`" + ` +
        ...
        ` + "`" + `input layout in src/Input/gen was not extended when the flag was added. Returning 0 here ` + "`" + ` +
  ...
  if (i < 0) {
    throw new Error(
      ` + "`" + `attrIndex: no wire byte exists for update attribute "${attr}" — the attr list this file was ` + "`" + ` +
        ...
        ` + "`" + `in src/Input/gen's input layout and regenerate, in the same commit as the decoder that ` + "`" + ` +
        ` + "`" + `reads it.` + "`" + `,
    );
  }
  return i;
