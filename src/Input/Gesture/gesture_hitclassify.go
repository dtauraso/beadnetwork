...
var hitClassifiers = map[string]func(d Deps, g *Fsm.GestureState, ev Codec.RawInputMsg){
	"handhold": func(d Deps, g *Fsm.GestureState, ev Codec.RawInputMsg) {
	...
	"node": func(d Deps, g *Fsm.GestureState, ev Codec.RawInputMsg) {
	...
	"empty": func(d Deps, g *Fsm.GestureState, ev Codec.RawInputMsg) {
		g.EmptyDown = true
		nodeGeoms, centerOf := d.MR.NodeGeoms(), d.MR.CenterOfNode
		g.BeginSphereRotation(d.UI.VP.Viewpoint, func() map[string]spatial.Vec3 { return nodemove.HeldCenters(nodeGeoms, centerOf) }, ev)
	},
