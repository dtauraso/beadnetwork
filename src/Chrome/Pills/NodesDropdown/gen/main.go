...
func writePathFiles(pathsDir string) error {
	...
	for _, name := range NodesDropdown.PillValueNames {
		file := name + ".bin"
		keep[file] = true
		rel := NodesDropdown.ValueRelPath(name)
...
func writeNames(path string) error {
	...
	b.WriteString("// GENERATED by src/Chrome/Pills/NodesDropdown/gen from PillValueNames in\n")
	...
	b.WriteString("// learns where from src/Chrome/Pills/NodesDropdown/paths/, which Go writes from\n")
