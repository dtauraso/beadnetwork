...
func writeOverlayPreamble(w *bufio.Writer, source string, needsFmt bool) {
	fmt.Fprintln(w, `// GENERATED by src/Chrome/OverlaysDropdown/gen — do not edit.`)
	...
	if needsFmt {
		...
		fmt.Fprintln(w, "\t\"fmt\"")
		fmt.Fprintln(w)
		fmt.Fprintln(w, "\tT \"github.com/dtauraso/wirefold/src/Trace\"")
		fmt.Fprintln(w, `)`)
	} else {
		fmt.Fprintln(w, `import (`)
		fmt.Fprintln(w, "\tT \"github.com/dtauraso/wirefold/src/Trace\"")
...
func writeOverlayStateGen(outPath string, flags []overlayFlag) error {
	...
	writeOverlayPreamble(w, "OVERLAY_FLAG_NAMES in src/messages.ts.", true)
...
func writeOverlayTablesGen(outPath string, flags []overlayFlag) error {
	var buf bytes.Buffer
	w := bufio.NewWriter(&buf)
	writeOverlayPreamble(w, "OVERLAY_FLAG_NAMES in src/messages.ts.", false)
