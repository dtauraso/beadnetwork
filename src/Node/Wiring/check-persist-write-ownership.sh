...
# PLACEMENT: src/Node/Wiring/**/*.go | writeJSONAtomic/entityReadModifyWrite calls must live in the owning file (node/edge/view mover), never elsewhere
...
if [[ ! -f "$PLUMBING_PATH" ]]; then
  ...
  echo "  This guard exempts src/Node/Wiring/jsonpersist as the shared write primitive; if it is" >&2
