...
# PLACEMENT: src/Node/Wiring/streamwire/stream_fds.go,src/extension/runCommand.ts,src/extension/runner/stream-fds.ts,src/extension/runner/spawn-layout.ts,src/extension/runner/stream-demux.ts | a new StreamKind must gain a WIREFOLD_STREAM_FDS env entry (runner/spawn-layout.ts builds the string, runCommand.ts's spawn env assigns it) and its own handle<Kind>Fd reader (runner/stream-demux.ts) in the ext host
