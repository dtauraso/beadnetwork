...
# PLACEMENT: src/Node/Wiring/**/*.go,src/Node/bead/*.go,src/schema/types.ts | a new SendRule const must also appear in the SEND_RULES array in types.ts (types.ts stays at schema/ top level — it is a registry, not part of the buffer-layout or input clusters)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

WIRING_DIR="$REPO_ROOT/src/Node/Wiring"
WIRE_DIR="$REPO_ROOT/src/Node/bead"
...
rules_from_go() {
  local go_files
  go_files=$(cd "$REPO_ROOT" && git ls-files 'src/Node/Wiring/**/*.go' 'src/Node/Wiring/**/*.go' 'src/Node/bead/*.go' 'src/Node/bead/**/*.go')
  [[ -z "$go_files" ]] && return
...
assert_nonempty "$GO_RULES" "SendRule consts (src/Node/bead/outport/send_rule.go)"
