...
node's own cycle (`clk.SleepCycle`, `src/Node/clock/clock.go`) are scaled by the scene's
...
(`applyUpdateTiltVector`, `src/Node/Wiring/stdinreader/dispatch_apply.go`), so a click is answered on the
