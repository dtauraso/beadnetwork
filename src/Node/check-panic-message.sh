...
# PLACEMENT: src/Node/**/*.go,src/Buffer/*.go,src/Trace/*.go | a panic must NAME the invariant it broke, not just the symptom
...
ROOTS=()
for d in src/Node src/Buffer src/Trace; do
  [ -d "$d" ] && ROOTS+=("$d")
done
