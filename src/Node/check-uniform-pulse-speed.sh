...
# PLACEMENT: src/Node/**/*.go | NewBeadRun must take no timing argument, so pulse speed cannot vary per run
...
DECL=$(git ls-files -z --cached --others --exclude-standard '*.go' \
  | xargs -0 grep -n "func NewBeadRun(" -- \
  | grep -v "_test.go" \
  || true)

if [[ -z "$DECL" ]]; then
  echo "uniform-pulse-speed: EMPTY — no NewBeadRun declaration found." >&2
  echo "  The constructor was renamed or removed; refusing a vacuous pass." >&2
  echo "  Update the grep in $0 to match the new shape." >&2
  exit 1
fi

PARAMS=$(printf '%s' "$DECL" | sed -n 's/.*func NewBeadRun(\([^)]*\)).*/\1/p')

if [[ -n "$PARAMS" ]]; then
  echo "uniform-pulse-speed: NewBeadRun takes an argument, so a run can be given its own speed:"
  printf '%s\n' "$DECL" | sed 's/^/  /'
  echo ""
...
echo "uniform-pulse-speed: clean — NewBeadRun takes no timing argument"
