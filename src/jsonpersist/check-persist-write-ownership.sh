...
if [[ ! -f "$PLUMBING_PATH" ]]; then
  ...
  echo "  This guard exempts src/jsonpersist as the shared write primitive; if it is" >&2
...
VIEW_OWNERS=("scene_camera_persist.go" "overlays_persist.go" "panels_persist.go" "scene_sphere_persist.go" "scene_selection_persist.go" "scene_speed_persist.go" "lattice.go")
...
if [[ ${#eligible_files[@]} -gt 0 ]]; then
  all_hits="$(grep -nE 'jsonpersist\.WriteJSONAtomic\(|jsonpersist\.ReadModifyWriteJSON\(|entityReadModifyWrite\(' "${eligible_files[@]}" 2>/dev/null || true)"
fi
