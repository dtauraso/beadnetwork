...
import (
	...
	"github.com/dtauraso/wirefold/src/runtopology/loadspec/gen/wiredefs"
)
...
func main() {
	...
	specPath := filepath.Join(srcRoot, "runtopology", "loadspec", "topo_spec.go")
	wireProps, err := wiredefs.ParseWirePropsFromFile(specPath)
	if err != nil {
		genpaths.Fatalf("parse wire props from topo_spec.go: %v", err)
	}

	outPath := filepath.Join(srcRoot, "runtopology", "loadspec", "wire-defs.ts")
