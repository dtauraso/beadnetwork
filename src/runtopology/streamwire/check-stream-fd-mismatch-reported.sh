...
# PLACEMENT: main.go,src/runtopology/edge_stream.go,src/runtopology/node_stream.go,src/runtopology/streamwire/stream_fds.go | every conditionally-wired per-owner StreamKind must have a named stream-fd mismatch report

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
cd "$REPO_ROOT"

KINDS_FILE="src/runtopology/streamwire/stream_fds.go"
