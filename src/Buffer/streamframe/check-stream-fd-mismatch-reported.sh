...
# PLACEMENT: main.go,src/Node/runtopology/edge_stream.go,src/Node/runtopology/node_stream.go,src/Buffer/streamframe/stream_fds.go | every conditionally-wired per-owner StreamKind must have a named stream-fd mismatch report

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
cd "$REPO_ROOT"

KINDS_FILE="src/Buffer/streamframe/stream_fds.go"
