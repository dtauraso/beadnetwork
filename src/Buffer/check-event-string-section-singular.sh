...
# PLACEMENT: src/Buffer/trace_buffer_block.go | bufLayoutBreadcrumb may declare at most one `<Name>Off uint32` free-form string section

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"
LAYOUT_FILE="$REPO_ROOT/src/Buffer/trace_buffer_block.go"
...
if [[ "$OFF_COUNT" -gt 1 ]]; then
  ...
  echo "  never a new *Off/*Len string blob. See src/Buffer/check-event-string-section-singular.sh."
  exit 1
fi
