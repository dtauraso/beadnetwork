...
node's own cycle (`clk.SleepCycle`, `src/Clock/clock.go`) are scaled by the scene's
...
(`applyUpdateTiltVector`, `src/runtopology/scenerun/edit_view_handlers.go`), so a click is answered on the
