...
- `kind` — required (non-empty), but **write-only/vestigial today**: `src/NodeKinds/gen/kindscan`
...
error**: the kind scanner (`src/NodeKinds/gen/kindscan`) fails the whole build, it is
