...
func generateCurveParams(repoRoot, srcRoot string) {
	...
	tsPath := filepath.Join(srcRoot, "schema", "buffer-layout", "curve-params.ts")
...
func generateShadingParams(repoRoot, srcRoot string) {
	...
	tsPath := filepath.Join(srcRoot, "schema", "buffer-layout", "shading-params.ts")
...
func generateColumnStreams(repoRoot, srcRoot string) {
	...
	goPath := filepath.Join(srcRoot, "schema", "buffer-layout", "column_streams_gen.go")
	if err := buflayout.WriteColumnStreamsGo(goPath, schema); err != nil {
		genpaths.Fatalf("write %s: %v", goPath, err)
	}
	tsPath := filepath.Join(srcRoot, "schema", "buffer-layout", "column-streams-gen.ts")
...
func generateBufferLayout(repoRoot, srcRoot string) {
	...
	bufferDir := filepath.Join(srcRoot, "schema", "buffer-layout")
...
func generateFrameTags(srcRoot string) {
	goPath := filepath.Join(srcRoot, "schema", "buffer-layout", "frame_tags.go")
	header, consts, err := parseFrameTags(goPath)
	if err != nil {
		genpaths.Fatalf("parse frame tags: %v", err)
	}
	tsPath := filepath.Join(srcRoot, "schema", "buffer-layout", "frame-tags.ts")
