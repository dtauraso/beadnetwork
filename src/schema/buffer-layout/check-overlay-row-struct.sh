...
# PLACEMENT: src/schema/buffer-layout/buffer_layout_gen_singletons.go,src/schema/buffer-layout/gen/buflayout/buffer_layout_singletons.go | SetOverlayRow must take one named OverlayRow struct, never positional same-typed scalars

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

GEN_FILE="$REPO_ROOT/src/schema/buffer-layout/buffer_layout_gen_singletons.go"
GENERATOR="$REPO_ROOT/src/schema/buffer-layout/gen/buflayout/buffer_layout_singletons.go"
