...
# PLACEMENT: src/schema/buffer-layout/buffer_layout_gen.go,src/schema/buffer-layout/buffer-layout.ts | BUF_LAYOUT_FINGERPRINT must match between the two generated layout files

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git rev-parse --show-toplevel)"

GO_FILE="$REPO_ROOT/src/schema/buffer-layout/buffer_layout_gen.go"
TS_FILE="$REPO_ROOT/src/schema/buffer-layout/buffer-layout.ts"
...
assert_nonempty "$FP_GO" "src/schema/buffer-layout/buffer_layout_gen.go"
...
if [[ "$FP_GO" != "$FP_TS" ]]; then
  ...
  echo "    $FP_TS"
  echo ""
  echo "Regenerate with: npm run gen:node-defs"
  exit 1
fi
