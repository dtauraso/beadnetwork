...
NAME="${1:-}"
DESC="${2:-}"
[ -n "$NAME" ] || die "usage: scripts/new-task.sh <short-kebab-name> [\"one-line description\"]"
...
[ -z "$(git status --porcelain)" ] || die "working tree is dirty — commit or park (scripts/wip.sh) before starting a new task"
...
[ -n "$DESC" ] || printf '  no description set — scripts/next.sh will show this branch unlabelled.\n  set one: git config branch.%s.description "..."\n' "$BRANCH"
