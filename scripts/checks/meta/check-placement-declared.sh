...
if [ "${#guards[@]}" -eq 0 ]; then
  echo "check-placement-declared: MISCONFIGURED — scripts/guard-list.sh named no guards; refusing to report success." >&2
  exit 1
fi
...
if [ "${#missing[@]}" -gt 0 ]; then
  ...
  echo
  echo "  # PLACEMENT: src/**/*.go | <what this guard demands of such a file>"
