...
while IFS= read -r spec; do DOCS+=("$spec"); done < <(git ls-files 'src/NodeKinds/*/SPEC.md' 2>/dev/null || true)
...
while IFS= read -r sym; do
  ...
  git grep -nE "(\`[^\`]*${sym}[^\`]*\`|<code[^>]*>[^<]*${sym}[^<]*</code>)" \
    -- '*.go' '*.ts' '*.tsx' CLAUDE.md MODEL.md 'src/NodeKinds/*/SPEC.md' 'docs/**/*.html' 'docs/**/*.html' 2>/dev/null \
    | head -3 | sed 's/^/      /' || true
done < "$TMP/ghosts.txt"
