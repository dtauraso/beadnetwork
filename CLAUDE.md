...
- **Verified work LANDS — but a USER-VISIBLE change is tested on its branch FIRST.** A task branch whose `scripts/stop-checks.sh` output is EMPTY merges to `main` without asking WHEN nothing about it changes what the person driving the editor sees or does. Anything that does — geometry, rendering, interaction, overlays, timing — stays on its branch until they have driven it and said it is good. Force-pushing your own task branch (after a rebase) and deleting it once merged are part of landing it, not separate permissions.
  - **Why the split, and why the old blanket rule was wrong:** there is ONE checkout now (the worktree workflow was removed), so `git checkout task/<name>` is what the editor runs. A branch is FULLY TESTABLE IN PLACE — it does not have to be merged to be seen. The old rule justified merging freely by "a change on a task branch is invisible to the person driving the editor", which was true only under the worktree layout and stopped being true when that layout went. Merging to make a change visible now means landing untested work into the tool they are using: in one session that shipped vanished edge beads, wrong animation colours, and a node that dragged backwards — each found by the user, in the editor, after the merge.
  - So: plumbing, refactors, guards, docs and tests land on green. Anything they will LOOK AT waits for them to look at it.
