...
([docs/model/entities.md](docs/model/entities.md); an edge's length is ONE INTEGER, its
bead-step count — `nodes/Wiring/edgegeom/chain_length.go`, on the lattice in
`nodes/wire/lattice/bead_lattice.go`). The active node kinds are the structs under `nodes/<Kind>/`.
