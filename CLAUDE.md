...
- Build and run before reporting a change as ready; verify output matches previous run. If verification fails, fix forward or revert — don't leave broken state on the branch. To exercise a TS change in the LIVE editor, run `npm run build` — the Stop hook does this automatically, but manual subagent verifications do not (why `tsc --noEmit` isn't enough is in the verify recipe below; stated once, there).
  ...
  - **`tools/guard-list.sh` is the ONE definition of which files are guards**, and both readers use it: the runner (`scripts/lib/guards.sh`) and the guard that audits the guards (`check-placement-declared.sh`). Depth is unbounded. Each previously hardcoded `tools/*/check-*.sh tools/*/*/check-*.sh`, so a guard three directories deep was invisible to BOTH — and silent non-coverage reads exactly like a passing guard. Do not re-add a second glob in either caller. Two consequences: (a) `check-*.sh` means "a runnable guard, executed directly", so a helper meant to be SOURCED must not be named that (`tools/network/trace/lib/gate-*.sh`) — a sourced library run as a guard would define its functions, exit 0, and count as a pass; (b) guard results are keyed by PATH (`guard_slug`), never basename, because two guards sharing a basename otherwise write the same `.out`/`.rc` and the `xargs -P` fan-out lets a passing one erase a failing one's result. `check-placement-declared.sh` fails on duplicate basenames to keep that unreachable.
