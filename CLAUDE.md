...
- **`Categories/Vector/`** — `Vec3`, `Segment`, seven operations; the ROOT imports only `math`,
  ...
  ALIASES it (`type Vec3 = Vector.Vec3`), never redefines it: an alias is the same type, so a
