...
1. **Never assert delivery.** A node calls `PlaceDrivenAt` and returns; the `PacedWire`
   ...
   (`memory/feedback/architecture/feedback_node_model_not_networking_handshake.md`). Split it: A's own emit
...
wrote those bytes. `memory/feedback/ux/feedback_headless_repro_verifies_persistence` records that
