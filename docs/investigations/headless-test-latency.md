...
is on the wire. This repo's topology has a self-feeding ring (`memory/feedback/architecture/feedback_edge_seed_
...
`memory/feedback/architecture/feedback_no_single_writer_bridge.md`, and no other test drives real fds. Keep all
three.
...
(`memory/feedback/debugging/feedback_ease_of_fix_is_confounded.md`).
...
idle interval is a knob, and per `memory/feedback/architecture/feedback_go_vs_coordinator_bias.md` a knob is worth
