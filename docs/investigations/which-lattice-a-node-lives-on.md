...
(`nodes/Wiring/layoutquant/quantized_move.go`), which has no grid in the drag path at all, so the two
...
`walkBeadPath` (`nodes/Wiring/layoutquant/quantized_move.go`) implements this: starting from the node's
...
- **The cartesian drag target has exactly ONE live producer.** `RootMove`'s only callers are
  ...
  `memory/project/locks-propagation/project_distance_group_decentralization_deadend.md`, which records that feature as
  built and reverted.
