...
- **Node goroutine.** One of SEVERAL serving the same node id, not "the" goroutine for that
  ...
  column — `nodes/rowevent/row_event.go`, `tools/topology-vscode/src/Buffer/streamframe/stream_events.go`,
  `tools/topology-vscode/src/Buffer/bufschema/layout.go` — which is a live 2x2 interior VISUAL grid position,
