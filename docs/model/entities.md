...
- **Wire (`PacedWire`).** Transport. A PASSIVE delay queue, not a
  ...
  beads is — its length is one integer, the bead-step count between the two nodes' tori
  (`edgegeom.EdgeStepCount`), never an arc. There is one owner
