...
// Length()/Normalize() — exactly the cartesian shortcut (nodes/Wiring/quantized_move.go's
// now-deleted walkBeadPath, and the two reverted drag-cell attempts before this one) that
// let a drag silently become a normalize-and-scale cartesian stride instead of index
// arithmetic on the lattice. With Dir and N as separate fields there is no field to
// normalize or measure a length from — the cartesian shortcut is structurally
// unavailable, not merely avoided by convention.
