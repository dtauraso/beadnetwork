...
//
// Both run inside a synctest bubble: `time` is the fake clock, so each sleep advances
// the tick by exactly the slept number of periods and the "original kept advancing"
// check is an equality rather than a >= with slack.
...
func TestRealClockCopyIndependent(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if got := orig.Tick(); got != 3 {
			t.Fatalf("original did not keep advancing while copy was frozen: Tick()=%d, want exactly 3", got)
		}
