...
`nodes/<Kind>/SPEC.md` and `parseSpecMD` parses only those sections from it. This file
carries everything else the generator does not read.
...
either node of a pair (`nodes/Wiring/moverreg/mover_registry.go`'s `FinalizeActors` never constructs one for an id
