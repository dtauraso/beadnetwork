...
|------|-----------|----------|----------|-------|
| OutCadence | out | chain | yes | broadcast output, gated like ToExcitatory (not required); its edge length is the one that sets this node's own emission cadence |
| ToExcitatory | out | chain | yes | fans the emitted value out to whatever samples and holds it; active when wired |
| FeedbackIn | in | chain | yes | receives step (1=advance, 0=hold index) from whatever computes it; enables feedback-ring mode when wired |
...
Plain emit path (FeedbackIn not wired): end-pop the working array each iteration, Fire and send each popped value on OutCadence. Exit when all values sent (or refill and continue if Repeat).

Feedback-ring path (FeedbackIn wired): iterate indefinitely.
1. Fire.
2. Send the current end value of the working array on OutCadence.
3. Block on FeedbackIn for a step value `s`.
