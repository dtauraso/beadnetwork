...
|------|-----------|----------|----------|-------|
| ToTime | out | chain |  | forwards Init values to the chain Time node |
...
Plain emit path (FeedbackIn not wired): end-pop the working array each iteration, Fire and send each popped value on ToTime. Exit when all values sent (or refill and continue if Repeat).

Feedback-ring path (FeedbackIn wired): iterate indefinitely.
1. Fire.
2. Send the current end value of the working array on ToTime.
