...
// gesture_graph.go (not over gesture-package behavior end to end — the dispatch package's
// own gesture_*_test.go files already cover that via MoveDispatch.HandleRawInput). It
// enumerates every path the table can express from gestPending and asserts the set of
