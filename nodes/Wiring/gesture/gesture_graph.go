...
// SAME gestPointerMove call). See gesture_handlers.go's gestPointerMove for the driver that
// walks this table.
//
...
// gestureEdge is one entry of the pending→phase commit table. Its action takes no *T.Trace:
// none of the three commit actions below (commitDragStart/commitHandholdStart/
// commitRotateStart) ever read the trace they used to be handed — a dead parameter
// diagnostics would flag on any one of them individually. Dropped here for the whole table
// rather than leaving each an unused-tr signature.
...
// or hit-testable, so the "port" raycast-hit kind that fed both commit edges can never fire — they
...
// old switch's implicit default (gestIdle, gestPending fell through to nothing). Unlike
// commitEdges' action, these DO use tr (applyOrbit/applyOrbitLocked hand it to the camera
// trace emit), so the type keeps it.
