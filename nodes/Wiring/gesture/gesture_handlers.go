...
// the FSM's transitions; the state they mutate is declared in gesturefsm. Pointer-down's hit
...
func gestPointerUp(d Deps, ev inputcodec.RawInputMsg) {
	...
	case g.Phase == gesturefsm.GestDragging:
		...
		applyNodeDragTarget(d.UI, func(id string, target vec3) bool { return lq.RootMove(ctx, nodeGeoms, id, target) }, ev) // final target flush
	...
	case g.Phase == gesturefsm.GestPending:
		...
		// selection. d.UI.Sel.Selected is the authoritative selection; Select() emits it so the
