...
func TestCommitNodeMoveLocalDrawsQuantizedNotRawTarget(t *testing.T) {
	...
	// (2) The committed center must equal the nearest UNION-model candidate cell
	// (chooseDragCandidate, "each bead is also a polar vector" — docs/bead-lattice.md) —
	// the positive half of the same assertion, pinning WHAT it should be, not just what it
	// shouldn't. This replaced an earlier version of this test that compared against
	// walkBeadPath's cartesian-stride walk, which itself replaced an even earlier
	// fixed-1-degree-angular-tick lattice point (offsetScenePolar(measureScalar(...))) that
	// UNDERSHOT a sideways move 7x-18x (the drag.jump probe finding).
	if !wantOk {
		t.Fatal("test assumes dst has a usable neighbour constraint (chooseDragCandidate ok=true)")
	}
	if d := got.Sub(want).Length(); d > 1e-6 {
		t.Fatalf("commitNodeMoveLocal's committed center does not match the chosen union candidate cell: got=%+v want=%+v", got, want)
	}
}
