...
//
// forwardDelta normally FANS a delta triple to every cascadeEdges neighbor except the
...
// dropped entirely. Every non-TimeStart node keeps the plain fan.
...
func TestForwardDeltaNonTimeStartFloodsAllExceptSender(t *testing.T) {
	...
	if len(got) != 2 || got[0] != "1" || got[1] != "4" {
		t.Fatalf("non-TimeStart mover: want fan to [1 4], got %v", got)
	}
}
