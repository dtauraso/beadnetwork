...
// geometry directly (PairNode today, the pair scene): via the dispatch core's bound
// a.deps.ClaimSelfDriveGeom, it records this node's id as self-driven in the dispatch
// core's mover registry (so finalizeActors never constructs a nodeMover for it at all —
// there is no flag on a mover to skip, because no mover is ever built for this id) and
// returns this node's own *nodeactor.NodeGeometry, which this method wraps into a handle
// the caller's own Update loop uses to run that geometry's per-cycle work (Step) and to
...
// bare test build with no loader (a.deps.ClaimSelfDriveGeom == nil) or if this node has no
// geometry entry, matching the nil-safe fallback every other closure in this file takes;
// every method on a nil *nodeactor.PairNodeSelf is itself a no-op.
func (a BuildArgs) ClaimSelfDrive() *nodeactor.PairNodeSelf {
	...
	// NodeMover.speedCh (finalizeActors) — otherwise this clock, copied by
	// ClaimSelfDriveGeom's own CopyClockSrc, never hears a later speed broadcast
	// (SceneTab.ClockDivisor or a live slider change) at all, even though the kind's own
	// SEPARATE clock (its own SpeedCh, polled in its Update loop) does. See
	// nodeactor.PairNodeSelf's own speedCh doc comment for the full defect this closes.
