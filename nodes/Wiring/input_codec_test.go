...
func TestOverlayFlagOrderMatchesFingerprint(t *testing.T) {
	want := []string{"tori", "scenePoles", "nodePoles", "selSpherePoles", "handholds", "labelsGlobal", "overlays", "cascadeLinks", "polarVectors"}
...
func TestSavePersistsCurrentOverlayState(t *testing.T) {
	root := t.TempDir()
	md := newMoveDispatch(map[string]nodeGeom{}, map[string]EdgeEndpoints{}, nil, nil, nil, wire.NewRealClock(), nil)
