...
// against it without duplicating the constant; the rule itself no longer lives in
// nodes/Wiring (see nodes/PairNode/node.go).
//
...
// node_geometry_stream.go's writeStreamFrame). A scene whose ring plane does NOT contain
// +y breaks that coincidence — θ would then measure something unrelated to the coplanar
// normal, and the rule would need to compare an actual dot(tilt, coplanarNormal) via the
// two integer indices' angles converted through anglesToWorldOffset, not thetaIdx alone.
