...
// vector direction (Buffer/layout.go's TopTiltVectorTheta): every node's
// topTiltVectorThetaIdx (node_mover.go) is an INTEGER count of this step,
...
// radians. Also the display step the per-node tilt-vector-angle panel
// (TiltVectorAnglePanel.tsx) formats its θ/φ readout against — matches
// CURVE_PARAM_TILT_VECTOR_ANGLE_STEP in the generated curve-params.ts, so the panel's
// "5π/12"-style label can never drift from Go's own step.
//
...
// vocabulary). math.TestTiltVectorAngleStepIsPiOver12 (curve_params_test.go) pins this
