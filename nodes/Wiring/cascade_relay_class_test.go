...
// either rule without a case here would silently keep reporting "flood" in the editor.
func TestCascadeRelayClass(t *testing.T) {
	for _, c := range []struct {
		...
		// flood — relay to every cascade neighbor except the sender.
		{"Input", 0},
		{"Time", 0},
		{"SelectLeft", 0},
		{"SelectRight", 0},
		// An unknown kind string floods, matching forwardDelta's own fall-through: the
