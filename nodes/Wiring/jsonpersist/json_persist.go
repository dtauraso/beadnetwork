// Package jsonpersist holds shared machinery for the domain persisters in nodes/Wiring
// (overlays, camera viewpoint, scene sphere, node position, local polars, port anchor).
package jsonpersist

// json_persist.go — shared machinery for the domain persisters in this package
// (overlays, camera viewpoint, scene sphere, node position, local polars, port anchor).
