...
// only *SceneSwitch, so the move drops the dispatch import entirely. The pure nodes/Wiring/
// scene helper tests that used to sit in the same file (SelectedSceneIndex/AnchorIsTabbed/
// ResolveScenePath/SceneTabNames) never exercised SelectScene and stayed behind in
// dispatch's own scene_tabs_test.go.
//
