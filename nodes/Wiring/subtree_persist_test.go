...
// the scene lattice — the nearest surviving intersection cell (quantized_move.go's
// chooseDragCandidate: "a legal cell must be a whole bead count from EVERY neighbour, not
// just one" model, docs/bead-lattice.md) — the raw target unchanged when quantizedLayout
// is off, or when nodeID has no usable neighbour constraint (chooseDragCandidate's
// ok=false free-move fallback, mirrored here). Test callers that assert convergence
