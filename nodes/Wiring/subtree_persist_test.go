...
// the scene lattice — the CELL (chooseDragCandidate) nearest target, in ONE neighbour's
// frame (dragCandidateCells' deterministic "lexicographically smallest neighbour id"
// rule) — the raw target unchanged when quantizedLayout is off. Test callers that assert
// convergence (pollDragConverged) must poll for THIS point, not the raw target, now that
// a committed drag is snapped rather than continuous
// (docs/which-lattice-a-node-lives-on.md). MUST be called BEFORE the drag commits: it
// reads the node's pre-drag LocalPolar/partnerCenters state, exactly what
// commitNodeMoveLocal itself reads at commit time — calling it again after the drag has
// already moved the node would read the POST-drag cell frame instead.
...
// — not recomputed here. quantizedDragTarget reads the node's CURRENT (pre-drag) cell
// frame, so calling it AFTER RootMove has already been issued races the dragged node's
// own mover goroutine: if the commit has landed by the time this function reads the
// state, "current" is already the POST-drag cell frame, and a fresh quantizedDragTarget
// call would pick a candidate relative to the wrong frame entirely. Passing want in
