...
// kind's own decision for a reset, from any starting index: it lands on 0 (the
...
// A plain adjust (the panel's ▲/▼ click, neither Reset nor Start) moves the tilt index by
...
func TestReceivedVectorRecordedEvenWhenNothingSteps(t *testing.T) {
	...
	if !n.ReceivedSet || n.ReceivedThetaIdx != arrived.ThetaIdx {
		t.Fatalf("the arrived direction must be recorded even when nothing steps; got set=%v theta=%d",
