...
// TimeStart is a clone of the Time kind (see nodes/Time) — identical behavior
// for now, split out so node 2's future divergence has a home of its own. It
// holds the most recently received bead and, on the NEXT receive, sends out the
// value it held from the PREVIOUS receive (a one-step "hold new, send old"
// delay) — a pure forwarder paced on its own clock. It broadcasts the held
// value concurrently to every ToNext output, waits out that broadcast's own
// traversal-tick budget as a processing window (never parking across a
...
func (in *TimeStart) consumeInput(clk clock.Clock, value int, held int) (newHeld int, windowActive bool, windowEndTick int64) {
	if in.Fire != nil {
		in.Fire()
	}
