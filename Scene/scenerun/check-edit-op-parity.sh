...
# PLACEMENT: Scene/scenerun/dispatch_edit.go,extension/messages.ts,Overlay/flags.ts,Chrome/Panels/Panel/flags.ts,Node/wire-gen.ts,Overlay/paths/,Chrome/Panels/Panel/paths/ | edit ops/update-kinds/overlay flags must stay listed identically on both sides of the bridge
...
OVERLAY_FLAGS_TS="$REPO_ROOT/Overlay/flags.ts"
PANEL_FLAGS_TS="$REPO_ROOT/Chrome/Panels/Panel/flags.ts"

HANDLE_MSG="$REPO_ROOT/Node/wire-gen.ts"

PANEL_STATE_GO="$REPO_ROOT/Chrome/Panels/Panel/panel_state.go"
...
assert_nonempty "$GO_OPS" "axis1 Go ops"
report_diff "$(comm -13 <(echo "$GO_OPS") <(echo "$TS_OPS"))" "Go ops" \
...
RENDER_READS=$(awk '/^var FlagNames = \[\]string\{/{p=1;next} p&&/^\}/{p=0} p' \
  "$REPO_ROOT/Overlay/flag_paths_gen.go" | grep -aoE '"[a-zA-Z]+"' | tr -d '"' | sort -u)
...
if [[ "$N_FLAGS" -ne "$N_READS" ]]; then
  ...
  echo "    (a flag was added/removed in Overlay/flags.ts without regenerating flag_paths_gen.go, so it has no slot in the block the renderer reads)"
  HITS=$((HITS+1))
fi
...
PANEL_PATH_FILES=$(awk '/^var FlagNames = \[\]string\{/{p=1;next} p&&/^\}/{p=0} p' \
  "$REPO_ROOT/Chrome/Panels/Panel/flag_paths_gen.go" | grep -aoE '"[a-zA-Z]+"' | tr -d '"' | sort -u)
