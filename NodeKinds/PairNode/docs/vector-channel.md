...
- **Coplanar normal**: a quarter turn (`Wiring.QuarterTurnPhiIdx`, 6 steps of
  ...
  sign. `coplanarNormal` (`NodeKinds/PairNode/vectors.go`) reads it straight off the tilt's own
  `quarter` link on the ring (`NodeKinds/PairNode/ring.go`) — the ring is built with that link
