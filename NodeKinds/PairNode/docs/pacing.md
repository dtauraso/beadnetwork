...
node's own cycle (`clk.SleepCycle`, `Clock/clock.go`) are scaled by the scene's
...
(`applyUpdateTiltVector`, `Scene/scenerun/edit_view_handlers.go`), so a click is answered on the
