...
(`Node/nodeactor/pair_node_self.go`)) — there is **no separate `NodeMover` goroutine** for
either node of a pair (`Node/moverreg/mover_registry.go`'s `FinalizeActors` never constructs one for an id
...
mover) carries THREE distinct edits, applied by `applyTiltEdit` (`NodeKinds/PairNode/edits.go`):
