...
Each `NodeKinds/<Kind>/SPEC.md` is the canonical description of one Go node kind. It is the source of truth that drives the Go runtime (firing rule + struct), the TSX render (port positions + label), and the AI-assisted parity check.
...
Leave it blank on a brand-new kind's first generation: `go generate ./...`
...
- `kind` — required (non-empty), but **write-only/vestigial today**: `NodeKinds/gen/kindscan`
...
- `shape`, `fill`, `stroke`, `width`, `height` — optional `NodeTypeDef`-compatible
  ...
  `Node/nodegeom/node_dims_gen.go` (used for port-to-port arc length), falling back to
  110×60 if omitted.

A missing `## View` section (or one missing the `Field`/`Value` table columns) is a **hard
error**: the kind scanner (`NodeKinds/gen/kindscan`) fails the whole build, it is
not skipped or treated as not-yet-migrated. Every `NodeKinds/<Kind>/` directory with a
