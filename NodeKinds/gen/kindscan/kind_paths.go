...
var networkSegments = [1]string{"Node"}

var kindsSegments = [1]string{"NodeKinds"}

func NetworkDir(repoRoot string) string {
	return filepath.Join(repoRoot, networkSegments[0])
}

func KindsDir(repoRoot string) string {
	return filepath.Join(repoRoot, kindsSegments[0])
}
