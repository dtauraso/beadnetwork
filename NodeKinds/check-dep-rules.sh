...
# PLACEMENT: NodeKinds/*/node.go,NodeKinds/*/*.go | a node-kind package may import only the shared spine (Wiring/gatecommon/bead/nodeapi/clock), never a sibling kind
...
NODES_DIR="$REPO_ROOT/NodeKinds"
if [ ! -d "$NODES_DIR" ]; then
  echo "check-dep-rules: MISCONFIGURED — $NODES_DIR not found; refusing a vacuous pass." >&2
  echo "  NodeKinds/ holds every node-kind package this guard exists to police; if it is gone," >&2
...
for dir in "$NODES_DIR"/*/; do
  ...
  imported="$(grep -rhoE "\"$MODULE/NodeKinds/[A-Za-z0-9_]+(/[A-Za-z0-9_/]+)?\"" "$dir" --include="*.go" 2>/dev/null \
      | sed -E "s#\"$MODULE/NodeKinds/([A-Za-z0-9_]+)(/[A-Za-z0-9_/]+)?\"#\1#" | sort -u || true)"

  while IFS= read -r dep; do
    [ -z "$dep" ] && continue
    [ "$dep" = "$kind" ] && continue
    is_spine "$dep" && continue
    echo "ILLEGAL DEP: NodeKinds/$kind imports sibling NodeKinds/$dep — kinds must couple only through the shared spine (Wiring/gatecommon)"
    fail=1
  done <<< "$imported"
done
