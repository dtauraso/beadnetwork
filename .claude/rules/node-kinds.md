...
`cmd/gen-node-defs` from `nodes/<Kind>/SPEC.md`; the key is the PascalCase Go kind name
verbatim. Node rendering is generic —
`src/Node/Shape/NodeInstances.tsx` draws every node from the
...
**Step 4 — run the generator.** `go run ./cmd/gen-node-defs` is the single generator
