...
**Step 1 — `NODE_DEFS`.** `NodeKinds/node-defs.ts` is generated by
`NodeKinds/gen` from `NodeKinds/<Kind>/SPEC.md`; the key is the PascalCase Go kind name
verbatim. Node rendering is generic —
`Ring/NodeShape/NodeInstances.tsx` draws every node from its block
...
**Step 3 — the Go package** under `NodeKinds/<Kind>/`.

**Step 4 — run the generator.** `go generate ./...` runs every generator
pipeline; it also refreshes `NodeKinds/kinds_gen.go`. That file's blank imports are what make a
package's `init()` — and therefore its `Wiring.RegisterBuilder` call — run at all.

`RegisterBuilder(kind, ports, build)` (`NodeKinds/kindapi/build_args.go`) populates
`kindreg.Registry` directly, so the registry is complete before `main` runs (`Wiring` here
aliases `NodeKinds/kindapi` — the kind-API package node kinds import, decoupled from the
dispatch core; `NodeKinds/kindreg` holds the registry itself — `Registry`, `NodeBuilder`,
...
`portwiring.KindPorts`, which `Node/nodegeom/gen` writes from the SPEC.md `## Ports`
