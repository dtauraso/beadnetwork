...
**Step 1 — `NODE_DEFS`.** `src/NodeKinds/node-defs.ts` is generated by
`src/NodeKinds/gen` from `src/NodeKinds/<Kind>/SPEC.md`; the key is the PascalCase Go kind name
verbatim. Node rendering is generic —
`src/Ring/NodeShape/NodeInstances.tsx` draws every node from its block
