...
the kind's own `portsdef` from `Categories/NodeKinds/<Kind>/SPEC.md`; the key is the PascalCase Go kind name
...
per-kind `kindPorts`, which the kind's own `portsdef` writes from the SPEC.md `## Ports`
...
`Categories/Node/Ports/kindports`, `Categories/Scene/Dispatch/kindports`) — reads every `a.In("X")`/`a.Out("X")`/`a.Broadcast("X")`/`a.DriveOut("X")`
