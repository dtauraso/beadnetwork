...
state (`viewstate.PanelState`, `src/Chrome/OverlaysDropdown/panels_persist.go`) — its
...
**`topology/` is one of several sibling SCENES**, not the only tree. `src/Node/Wiring/scene/scene.go`'s
...
- A node writes its own `position/local-polars` (path construction in
  ...
  `src/Node/wire/lattice/bead_lattice.go`); this bullet used to list it as a second thing the mover writes.
...
`src/Node/Wiring/check-scene-path-resolution.sh` (who may construct a `nodes/` path).
...
extension host reader (`src/runCommand.ts`'s `readCounts`) and the Go
