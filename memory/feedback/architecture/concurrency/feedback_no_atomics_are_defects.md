...
**Rule:** In the concurrent node network (`nodes/`, `src/schema/buffer-layout/`, `src/Trace/`) there is ZERO shared mutable state. Each piece of state is owned by exactly one goroutine; anything another goroutine needs is SENT to it as a value, not read from a shared location. Under this model a synchronization primitive is a **defect marker**: if code reaches for `sync.Mutex`/`RWMutex` OR `sync/atomic`, it shared something it should have owned. Do not add either "for safety" or "for a fast lock-free read" — restructure so there is no cross-goroutine read at all.
