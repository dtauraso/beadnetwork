...
Concretely: if `Edit` on a guarded file is blocked, do not:
...
Canonical example: PreToolUse hooks guard Go source files or other sensitive paths. They fire on `Edit|Write|MultiEdit` and on `Bash` commands that touch guarded paths with write verbs. Both paths are intentional — a subagent previously bypassed the Edit guard by invoking `python3` from Bash.
