...
- `check-breadcrumb-label-registered.sh` — hardcoded `src/Trace/Trace.go`; splitting that file would have blinded it. Fixed to scan `src/Trace/*.go` (Trace.go itself was deleted 2026-08-19 as dead). See [[guards-hardcoding-single-file-break-on-split]].
