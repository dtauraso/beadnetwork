...
- `check-breadcrumb-label-registered.sh` — hardcoded `tools/topology-vscode/src/Trace/Trace.go`; splitting that file would have blinded it. Fixed to scan `tools/topology-vscode/src/Trace/*.go`. See [[guards-hardcoding-single-file-break-on-split]].
