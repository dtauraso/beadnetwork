...
- `check-breadcrumb-label-registered.sh` — hardcoded `tools/topology-vscode/Trace/Trace.go`; splitting that file would have blinded it. Fixed to scan `tools/topology-vscode/Trace/*.go`. See [[guards-hardcoding-single-file-break-on-split]].
