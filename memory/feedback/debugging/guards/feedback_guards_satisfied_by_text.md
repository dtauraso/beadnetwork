...
- `check-breadcrumb-label-registered.sh` — hardcoded `src/schema/buffer-layout/Trace.go`; splitting that file would have blinded it. Fixed to scan `src/schema/buffer-layout/*.go` (Trace.go itself was deleted 2026-08-19 as dead). See [[guards-hardcoding-single-file-break-on-split]].
