...
- `check-breadcrumb-label-registered.sh` — hardcoded `src/Trace/Trace.go`; splitting that file would have blinded it. Fixed to scan `src/Trace/*.go`. See [[guards-hardcoding-single-file-break-on-split]].
