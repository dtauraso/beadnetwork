...
**Third signature, 2026-08-18 — an ON-DISK FORMAT change makes the stale host fail HARD, not stale.** The per-value persistence split moved `topology/counts.json` to `counts/nodes.json`+`counts/edges.json`. The running host still called the old `readCounts`, threw ENOENT, and `prepareRunLayout` returned undefined — so Go was never spawned and nodes, edges AND the scene tab strip were all gone at once. `.probe/go-errors.jsonl` named it exactly, and the give-away is that the error text is the OLD code's message naming the OLD path. **Do not "unblock" this by recreating the old file** — that reintroduces the duplicate the split existed to remove; restarting VS Code is the fix, and quitting/reopening works when Reload Window does not.
