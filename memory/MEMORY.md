...
- [project_node_color_vocab.md](project_node_color_vocab.md) — David's node-kind nicknames: "time nodes" = Time (renamed from HoldNewSendOld), "and nodes" = SelectRight/SelectLeft (renamed from the old WindowAndInhibit*Gate spelling)
...
- [project_v0_cost_calibration.md](project_v0_cost_calibration.md) — per-phase ratios size the GENERATION step only (mechanical ~10%, hardening ~12%, refactor ~15–20%); rearchitecting several times after v0 cost ~$1.8k in API usage
...
- [project_wire_is_straight_line_not_chain.md](project_wire_is_straight_line_not_chain.md) — Original bead-item chain's O(N²) lag was human-clock gating + momentum-free midpoint relaxation, not "a chain of beads"; the fix primitive (nodes/wire/bead_actor.go) now has a production call site (nodes/Wiring/bead_chain.go) for the source node's own drag
...
- [project_theta_phi_tilted_camera.md](project_theta_phi_tilted_camera.md) — Apparent θ mismatch is usually φ seen through a tilted camera; measure θ from world +y. Layout uses a moving local pole (localPole), so the chart singularity is never reached
- [project_torus_colinearity_future_equation.md](project_torus_colinearity_future_equation.md) — port∈torus is now polar-only (no node-center z-drag; ended the (3,r)=(6,r) blow-up); ports snap to their rings and the edge bends between them — colinearity is not measured/enforced, no rebuild planned
- [feedback_no_atomics_are_defects.md](feedback_no_atomics_are_defects.md) — An atomic (like a mutex) in the network is a shared-state defect, not a tool; correct = ownership + message-passing, zero shared memory (enforced by check-no-network-locks.sh, empty allowlist)
- [feedback_debug_data_before_theory.md](feedback_debug_data_before_theory.md) — Live editor/geometry bug: get data first (probe logs, bisect a known-good build, instrument+repro) BEFORE hypothesizing; theorizing-first produced two confidently-wrong camera fixes this session
- [Distance-group decentralization dead end](project_distance_group_decentralization_deadend.md) — built, reverted; children don't follow parents for free
